...
utils.add_logo()
sku_list = z_score.iloc[:, 0].tolist()
...
def process_data(raw, calendar, zscore, store_id):
    ...
    sales.loc[:, "Feature"] = np.maximum.reduce(
        [sales["Feature1"], sales["Feature2"], sales["Feature3"], sales["Feature4"]]
    )

    ## Build rolling L8W Avg Sales & L7W Sum Sales
    sales.sort_values(by=["SKU", "Time_ID"], inplace=True)
    sales["Lag8w_avg_sls"] = sales.groupby("SKU")["Sales"].transform(
        lambda x: x.rolling(window=8, min_periods=1).mean()
    )
    sales["Lag7w_sum_sls"] = sales.groupby("SKU")["Sales"].transform(
    ...
    med_prices = (
        med_prices.groupby(["SKU", "Year"])["Price"]
        .median()
...
with st.form(key="all_inputs_form"):
    ...
    with c1:
        week_horizon = st.selectbox('Week Horizon',[8, 4, 16])
