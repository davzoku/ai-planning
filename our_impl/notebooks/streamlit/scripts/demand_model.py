...
class DemandModel:
    ...
    def fitness_demand(self, ga_output: pd.DataFrame, sku_list: list, start: float, period: int) -> float:
        ...
        sales_hist = self.sales[(self.sales['SKU'].isin(sku_list)) & (self.sales['Time_ID']>=histr-period-5) & (self.sales['Time_ID']<=histr)].copy()
        ...
        comp_matrix_columns = [f'{sku}_{promo}' for sku in self.all_skus for promo in ['Discount', 'Display', 'Feature', 'Sales']]
        ...
        for week in range(start, end):
            ...
            events_tmp = self.events[self.events['Time_ID']==week].drop(columns = 'Time_ID').copy()
            ...
            prices_tmp = self.prices[(self.prices['SKU'].isin(sku_list)) & (self.prices['Year']==year)]['med_price'].values
...
revenue = dm.fitness_demand(ga_output, sku_list, start, period)
print(revenue)